...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('connection_attempts', (table) => {
    ...
    table.string('node_id', 36).nullable().references('id').inTable('vpn_nodes').onDelete('SET NULL')
...
export async function down(knex: Knex): Promise<void> { await knex.schema.dropTableIfExists('connection_attempts') }
