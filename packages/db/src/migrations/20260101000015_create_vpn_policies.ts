...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('vpn_policies', (table) => {
    ...
    table.string('group_id', 36).nullable().references('id').inTable('groups').onDelete('CASCADE')
    table.string('node_id', 36).nullable().references('id').inTable('vpn_nodes').onDelete('CASCADE')
...
export async function down(knex: Knex): Promise<void> { await knex.schema.dropTableIfExists('vpn_policies') }
