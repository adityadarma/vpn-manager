...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('session_activities', (table) => {
    ...
    table.string('session_id', 36).notNullable().references('id').inTable('vpn_sessions').onDelete('CASCADE')
...
export async function down(knex: Knex): Promise<void> { await knex.schema.dropTableIfExists('session_activities') }
